...
func NewPocDBV1(rootPath string, ordinal int64, pubKey *pocec.PublicKey, bitLength int) (*PocDBV1, error) {
	...
	return mdb.(*PocDBV1), nil
}

func (mdb *PocDBV1) executePlot(result chan error) {
	var err error
	...
	logging.CPrint(logging.INFO, "start plotting",
		logging.LogFormat{"bit_length": mdb.bitLength, "pub_key": hex.EncodeToString(mdb.pubKey.SerializeCompressed())})
	if err = mdb.prePlotWork(cache); err != nil {
		...
		logging.CPrint(logging.ERROR, "pre plot fail",
			logging.LogFormat{"bit_length": mdb.bitLength, "pub_key": hex.EncodeToString(mdb.pubKey.SerializeCompressed()), "err": err})
		return
	}
...
func (mdb *PocDBV1) prePlotWork(cache *MemCache) error {
...
func (mdb *PocDBV1) plotWork(cache *MemCache) error {
