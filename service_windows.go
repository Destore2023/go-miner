...
const (
	// svcName is the name of sukhavati miner service.
	svcName = "sktsvc"
	...
	svcDisplayName = "Sukhavati Miner Service"

	// svcDesc is the description of the service.
	svcDesc = "Downloads and stays synchronized with the Sukhavati block " +
...
// logServiceStartOfDay logs information about miner when the main server has
...
// minerService houses the main service handler which handles all service
// updates and launching minerMain.
...
// long-running minerMain (which is the real meat of skt), handles service
// change requests, and notifies the service control manager of changes.
func (s *minerService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start minerMain in a separate goroutine so the service can start
	...
	for {
		...
		case c := <-r:
			...
			case svc.Stop, svc.Shutdown:
				...
				// minerMain to exit in the goroutine above which
...
// installService attempts to install the miner service.  Typically this should
...
// removeService attempts to uninstall the miner service.  Typically this should
...
// startService attempts to start the miner service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &minerService{})
	if err != nil {
