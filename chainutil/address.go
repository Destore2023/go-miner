...
import (
	...
	"github.com/Sukhavati-Labs/go-miner/chainutil/base58"
...
func DecodeAddress(addr string, defaultNet *config.Params) (Address, error) {
	...
	// (hrp) followed by '1'. For skt mainnet the hrp is "sk", and for
	...
	if oneIndex > 1 {
		...
		if config.IsBech32SegwitPrefix(prefix) {
			...
			// The HRP is everything before the found '1'. sk
...
// NewAddressPubKeyHash returns a new AddressPubKeyHash.  pkHash len must be 20
// bytes.
...
// differs with the format.  At the time of this writing, most skt addresses
...
func NewAddressPoolScriptHash(witnessProg []byte, net *config.Params) (*AddressWitnessScriptHash, error) {
	return newAddressWitnessScriptHash("pool", 1, witnessProg)
}
