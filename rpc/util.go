...
type orphanTxDescList []*chainutil.Tx
...
func AmountToString(m int64) (string, error) {
	if m > chainutil.MaxAmount().IntValue() {
	...
	u, err = u.AddUint(consensus.SukhavatiPerSkt)
	if err != nil {
		return "", err
	}
	s := u.String()
...
func StringToAmount(s string) (chainutil.Amount, error) {
	s1 := strings.Split(s, ".")
	if len(s1) > 2 {
		return chainutil.ZeroAmount(), fmt.Errorf("illegal number format")
	}
	...
	if len(s1) == 2 {
		...
		if len(sFrac) > 8 {
			return chainutil.ZeroAmount(), fmt.Errorf("precision is too high")
		}
	}
	...
	if i < 0 || uint64(i) > consensus.MaxSkt {
		return chainutil.ZeroAmount(), fmt.Errorf("integral part is out of range")
	}
	...
	u := safetype.NewUint128FromUint(consensus.SukhavatiPerSkt)
	u, err = u.MulInt(i)
	if err != nil {
		return chainutil.ZeroAmount(), err
	}
	...
	total, err := chainutil.NewAmount(u)
	if err != nil {
		return chainutil.ZeroAmount(), err
	}
	return total, nil
