...
import (
	...
	gw "github.com/Sukhavati-Labs/go-miner/rpc/proto"
...
func getIPAccessControlFunc(whitelist []string, lanPrefix []string) (func(addr string) bool, error) {
	...
	var fn = func(addr string) bool {
		...
		if err != nil {
			logging.CPrint(logging.WARN, "rpc fail to resolve http request.RemoteAddr", logging.LogFormat{"request_addr": addr, "err": err})
			return false
		}
...
func accessControlHandler(h http.Handler, isAllowedAddress func(addr string) bool) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		if !isAllowedAddress(req.RemoteAddr) {
			logging.CPrint(logging.WARN, "rpc received request from forbidden address", logging.LogFormat{"remote_addr": req.RemoteAddr, "url_path": req.URL.Path})
