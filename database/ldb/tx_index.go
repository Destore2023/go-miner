...
var (
	...
	//  | key prefix | script hash | lower boundary |
	//   -------------------------------------------
	//  |   3 bytes  |  32 bytes   |     8 bytes    |
	//                     |
	//                    \|/
	//  |       bitmap      | index  |  count  | tx offset | tx length | (4 + 4) x N | index  |  count  | tx offset | tx length | (4 + 4) x N |
	//  --------------------------------------------------------------------------------------------------------------------------------------
	//  |  4 bytes [31...0] | 1 byte | 2 bytes |  4 bytes  |  4 bytes  |     ...     | 1 byte | 2 bytes |  4 bytes  |  4 bytes  |     ...     |
	//
...
func calcHTSLowBoundary(blockHeight uint64) uint64 {
	return blockHeight / htsBoundaryDistance * htsBoundaryDistance
}

func calcSTLLowBoundary(blockHeight uint64) uint64 {
	return blockHeight / stlBoundaryDistance * stlBoundaryDistance
}
...
func encodeHTSKey(blockHeight uint64, scriptHash [sha256.Size]byte) (key []byte, lowBound uint64) {
	lowBound = calcHTSLowBoundary(blockHeight)
...
func encodeHTSSearchKeyPrefix(blockHeight uint64) (prefix []byte, lowBound uint64) {
	lowBound = calcHTSLowBoundary(blockHeight)
...
func encodeSTLKey(blockHeight uint64, scriptHash [sha256.Size]byte) (key []byte, lowBound uint64) {
	lowBound = calcSTLLowBoundary(blockHeight)
...
func (db *ChainDb) submitAddrIndex(height uint64, addrIndexData *database.AddrIndexData) error {
	...
	for addrKey, indexes := range txAddrIndex {
		...
		htsOldValue, err := db.localStorage.Get(htsKey)
		if err != nil {
		...
		stlOldValue, err := db.localStorage.Get(stlKey)
		if err != nil {
			if err != storage.ErrNotFound {
				return err
			}
	...
	for scriptHash, btxss := range gtxSpentIndex {
		...
		for _, btxs := range btxss {
			...
			btxIndex := &bindingTxIndex{
				scriptHash:  scriptHash,
				blockHeight: btxs.BTxBlockHeight,
			...
			if _, exist := delCountMap[btxs.BTxBlockHeight]; !exist {
				delCountMap[btxs.BTxBlockHeight] = 1
			} else {
				delCountMap[btxs.BTxBlockHeight]++
			}
		}
		...
		for btxHeight, delCount := range delCountMap {
			...
			iter := db.localStorage.NewIterator(storage.BytesPrefix(btxDeleteIndex))
			defer iter.Release()
...
func (db *ChainDb) deleteAddrIndex(height uint64) error {
	...
	iter := db.localStorage.NewIterator(storage.BytesPrefix(shPrefix))
	defer iter.Release()
	for iter.Next() {
		key := iter.Key()
		value := iter.Value()
	...
	btxIter := db.localStorage.NewIterator(storage.BytesPrefix(btxShPrefix))
	...
	for btxIter.Next() {
		...
		deleteGtxIter := db.localStorage.NewIterator(storage.BytesPrefix(deletePrefix))
	...
	btxsIter := db.localStorage.NewIterator(storage.BytesPrefix(btxsPrefix))
...
func (db *ChainDb) CheckScriptHashUsed(scriptHash []byte) (bool, error) {
	...
	iter := db.localStorage.NewIterator(storage.BytesPrefix(prefix))
	defer iter.Release()
	if iter.Next() {
		if err := iter.Error(); err != nil {
			return false, err
...
func (db *ChainDb) checkAddrIndexVersion() error {
	...
	data, err := db.localStorage.Get(addrIndexVersionKey)
	if err != nil {
		return err
	}
