...
func (db *ChainDb) CheckTxIndex_1_1_0() error {
	...
	for height := uint64(0); height <= bestHeight; height++ {
		_, buf, err := db.getBlockByHeight(height)
		if err != nil {
			return err
		}
			...
			if err == nil {
				for _, sptx := range list {
					if sptx.blockHeight == height {
						if sptx.numTxOut <= 0 || sptx.numTxOut%8 != 0 || sptx.numTxOut < len(tx.TxOut) {
		...
		if err = checkTxIndexRelated(db, height, blockRelatedScriptHash); err != nil {
			return err
		}
...
func checkTxIndexRelated(
	db *ChainDb,
	height uint64,
	blockRelatedScriptHash map[wire.Hash]map[wire.TxLoc]bool,
) error {
	for scriptHash, txlocs := range blockRelatedScriptHash {
		// check "HTS"
		htsKey, htsBoundary := encodeHTSKey(height, scriptHash)
		htsValue, err := db.localStorage.Get(htsKey)
		...
		stlValue, err := db.localStorage.Get(stlKey)
