...
func TestLevelDb_FetchTxByShaList(t *testing.T) {
	...
	for i, hash := range txList {
		assert.Equal(t, hash, txReply[i].TxSha)
	}
}
...
func TestLevelDb_FetchTxBySha(t *testing.T) {
	...
	assert.Equal(t, txReply[0].TxSha, hash1)
}
...
func TestLevelDb_ExistsTxSha(t *testing.T) {
	...
	exist, err := db.ExistsBlockSha(blockHash)
	assert.Nil(t, err)
	assert.True(t, exist)
}
...
// 	err = db.DeleteAddrIndex(blockSha, 9)
// 	assert.Nil(t, err)
