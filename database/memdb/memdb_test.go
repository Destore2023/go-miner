...
func TestClosed(t *testing.T) {
	...
	// err = db.SubmitBlock(chainutil.NewBlock(config.ChainParams.GenesisBlock))
	err = db.InitByGenesisBlock(chainutil.NewBlock(config.ChainParams.GenesisBlock))
	if err != nil {
	...
	if _, err := db.ExistsBlockSha(genesisHash); err != leveldb.ErrClosed {
...
func TestMemDb(t *testing.T) {
	...
	// test empty function
	// banlist.go test
	fpk, height, err := db.FetchFaultPubKeyBySha(&wire.Hash{})
	assert.Equal(t, storage.ErrNotFound, err)
	assert.Equal(t, uint64(0), height)
	assert.Nil(t, fpk)

	fpks, err := db.FetchFaultPubKeyListByHeight(0)
	assert.Nil(t, err)
	assert.Equal(t, 0, len(fpks))

	exists, err := db.ExistsFaultPubKey(&wire.Hash{})
	assert.False(t, exists)
	assert.Nil(t, err)

	fpks, heights, err := db.FetchAllFaultPubKeys()
	assert.Nil(t, err)
	assert.Equal(t, 0, len(fpks))
	...
	if _, err := db.ExistsBlockSha(&genesisHash); err != nil {
		t.Errorf("ExistsSha: Unexpected error: %v", err)
	}
	...
	if blockHashes, err := db.FetchHeightRange(0, 1); err != nil {
		t.Errorf("FetchHeightRange: unexpected error %v", err)
	} else {
		assert.Equal(t, 1, len(blockHashes))
	}
