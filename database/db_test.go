...
func TestEmptyDB(t *testing.T) {
	for _, dbType := range database.SupportedDrivers() {
...
func TestAddDuplicateDriver(t *testing.T) {
	supportedDBs := database.SupportedDrivers()
...
func TestCreateOpenFail(t *testing.T) {
	...
	// error.
	_, err := database.Create(dbType, "createfailtest")
	if err != openError {
	...
	// error.
	_, err = database.Open(dbType, "openfailtest")
	if err != openError {
		t.Errorf("TestCreateOpenFail: expected error not received - "+
			"got: %v, want %v", err, openError)
		return
...
func TestCreateOpenUnsupported(t *testing.T) {
	...
	// expected error.
	dbType := "unsupported"
	_, err := database.Create(dbType, "unsupportedcreatetest")
	...
	// Ensure opening a database with the new type fails with the expected
	// error.
	_, err = database.Open(dbType, "unsupportedopentest")
	if err != database.ErrDbUnknownType {
		t.Errorf("TestCreateOpenUnsupported: expected error not "+
			"received - got: %v, want %v", err, database.ErrDbUnknownType)
		return
