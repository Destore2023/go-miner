...
import (
	...
	"github.com/Sukhavati-Labs/go-miner/p2p/upnp"
...
func getUPNPExternalAddress(externalPort, internalPort int) (*NetAddress, error) {
	...
	externalPort, err = nat.AddPortMapping("tcp", externalPort, internalPort, "upnp tcp", 0)
	if err != nil {
		return nil, errors.Wrap(err, "could not add tcp UPNP port mapping")
	}
	externalPort, err = nat.AddPortMapping("udp", externalPort, internalPort, "upnp udp", 0)
	if err != nil {
...
//DefaultListener Implements miner server Listener
