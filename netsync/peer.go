...
import (
	...
	"github.com/Sukhavati-Labs/go-miner/chainutil/ccache"
	...
	"github.com/Sukhavati-Labs/go-miner/p2p/trust"
...
func (p *peer) isRelatedTx(tx *chainutil.Tx) bool {
...
func (p *peer) sendBlock(block *chainutil.Block) (bool, error) {
...
func (p *peer) sendTransactions(txs []*chainutil.Tx) (bool, error) {
	for _, tx := range txs {
...
func (ps *peerSet) broadcastMinedBlock(block *chainutil.Block) error {
...
func (ps *peerSet) broadcastNewStatus(bestBlock, genesisBlock *chainutil.Block) error {
	ps.mtx.RLock()
	defer ps.mtx.RUnlock()
...
func (ps *peerSet) broadcastTx(tx *chainutil.Tx) error {
	msg, err := NewTransactionMessage(tx)
	if err != nil {
