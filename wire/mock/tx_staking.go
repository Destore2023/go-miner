...
func (m *lUtxoMgr) constructRewardTxOut(blockHeight uint64, tx *wire.MsgTx) (out []*stakingRank, err error) {
	...
	for stakingAddr, utxos := range m.addr2utxos {
		...
		for _, UTXO := range utxos {
			...
			if UTXO.blockHeight+UTXO.maturity <= blockHeight { // expired
				continue
			}

			if lrank.txout == nil {
				addr, err := chainutil.DecodeAddress(stakingAddr, &config.ChainParams)
				if err != nil {
					return nil, err
				}
				...
				//
				normalAddr, err := chainutil.NewAddressWitnessScriptHash(addr.ScriptAddress(), &config.ChainParams)
				if err != nil {
					return nil, err
				}
			...
			lrank.weight += UTXO.value * int64((UTXO.blockHeight + UTXO.maturity - blockHeight))
		}
		if lrank.txout != nil {
...
func (m *lUtxoMgr) constructStakingTx(blockHeight uint64, blockIndex int) (mtx *wire.MsgTx, fee int64, err error) {
	...
	for ; cur <= stop; cur++ {
		...
		for _, utxo := range utxos {
			...
			if utxo.blockHeight+utxo.maturity > blockHeight {
				continue
			}
	...
	if sufficient {
		...
		addr, err = chainutil.NewAddressStakingScriptHash(addr.ScriptAddress(), &config.ChainParams)
		...
		// construct full tx
		mtx, err = m.createStakingTx(blockHeight, inputs, outputs, blockIndex)
		if err != nil {
			return nil, 0, err
		}
		if output.Value == 0 {
