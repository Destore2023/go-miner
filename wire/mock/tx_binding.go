...
type bindingUtxo struct {
	utxo           *utxo
	bindingAddress chainutil.Address
}
...
func (m *gUtxoMgr) calcCoinbaseOut(blockHeight uint64, coinbaseTx *wire.MsgTx, pocPk *pocec.PublicKey,
	...
	gValue := chainutil.ZeroAmount()
	var witness [][]byte

	req, ok := bindingRequiredAmount[bitLength]
	if ok {
		for _, gutxo := range m.getGuaranties(addr.EncodeAddress(), blockHeight) {
	...
	return CalcBlockSubsidy(blockHeight, &config.ChainParams, gValue, numRank, bitLength)
}

func (m *gUtxoMgr) getGuaranties(addr string, blockHeight uint64) []*bindingUtxo {
	list := make([]*bindingUtxo, 0)
	for _, gutxo := range m.pocAddr2utxos[addr] {
		if gutxo.utxo.spent {
			continue
		}
		if gutxo.utxo.blockHeight+gutxo.utxo.maturity > blockHeight { // unusable
			continue
		}
...
func (m *gUtxoMgr) constructBindingTx(blockHeight uint64, blockIndex int) (mtx *wire.MsgTx, fee int64, err error) {
	...
	if sufficient {
		...
		// construct full tx
		mtx, err = m.createBindingTx(blockHeight, inputs, outputs, blockIndex)
		if err != nil {
			return nil, 0, err
		}
		if output.Value == 0 {
