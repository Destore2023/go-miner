...
func mockGenesisBlock(timestamp int64, target *big.Int) *wire.MsgBlock {
	...
	var coinbasePayload = func(height int64) []byte {
		...
		sb = sb.AddData([]byte("SKT TESTNET"))
	...
	// fillCoinbaseCommitment(chainutil.NewBlock(genesisBlock))
...
func mockFillGenesisHeader(block *wire.MsgBlock, timestamp int64, target *big.Int) {
	...
	sig2, err := key.Sign(wire.HashB([]byte("SKT TESTNET")))
	if err != nil {
		panic(err)
	}
