...
func (c *Chain) constructTxs1(blockHeight uint64) ([]*wire.MsgTx, map[TxType]int, int64, error) {
	...
	for len(txs) < c.opt.MinNormalTxPerBlock {
		mtx, fee, spentBroUtxo, noUtxo, err := c.constructNormalTx(blockHeight, len(txs)+1, false)
		if err != nil {
	...
	for i := 0; i < max; i++ {
		...
		case TxNormal, Txwithdrawal:
			mtx, fee, spentBroUtxo, noUtxo, err := c.constructNormalTx(blockHeight, len(txs)+1, tp == Txwithdrawal)
			if err != nil {
				return nil, nil, 0, err
			}
			if mtx == nil || noUtxo {
			...
			totalFee += fee
		case TxStaking:
			mtx, fee, err := c.lUtxoMgr.constructStakingTx(blockHeight, len(txs)+1)
			if err != nil {
				return nil, nil, 0, err
			}
			if mtx == nil {
			...
			totalFee += fee
		case TxBinding:
			mtx, fee, err := c.gUtxoMgr.constructBindingTx(blockHeight, len(txs)+1)
			if err != nil {
				return nil, nil, 0, err
			}
			if mtx == nil {
...
func (c *Chain) constructNormalTx(blockHeight uint64, blockIndex int, withdrawal bool) (mtx *wire.MsgTx,
	...
	allowSpendBroUtxo := !withdrawal && (int64(blockIndex)+time.Now().UnixNano())%2 == 0
...
out:
	for _, owner := range owners {
		...
		for _, utxo := range utxos {
			...
			if utxo.blockHeight+utxo.maturity > blockHeight { // coinbase or staking output
				continue
			}
			if utxo.blockHeight == blockHeight {
			...
			val1 := utxo.value
			val2 := int64(0)
			if val1 >= 10000000000 && time.Now().UnixNano()%10 < 1 && !c.opt.NoNormalOutputSplit { // must >=100 skt
				val2 = val1 / 2
				val1 = val1 - val2
			}
			output := TxOutput{
	...
	if len(inputs) > 0 {
		// construct full tx
		mtx, err = c.constructTx(blockHeight, inputs, outputs, blockIndex)
		if err != nil {
			return nil, 0, false, false, err
		}
	} else {
...
func (c *Chain) signWitnessTx(tx *wire.MsgTx) error {
	...
	for i, txIn := range tx.TxIn {
		...
		getScript := txscript.ScriptClosure(func(addr chainutil.Address) ([]byte, error) {
...
func newWitnessScriptAddress(pubkeys []*btcec.PublicKey, nrequired int, net *config.Params) ([]byte, chainutil.Address, error) {
