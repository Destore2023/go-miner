...
func TestNewMockedChain(t *testing.T) {
	...
	for _, block := range blocks {
		height := block.Header.Height
		// t.Log("height", height, "hash", block.BlockHash(), "tx num", len(block.Transactions))
		for j, tx := range block.Transactions {
			...
			for _, in := range tx.TxIn {
				if !findUTxO(in.PreviousOutPoint.Hash, int(in.PreviousOutPoint.Index), height) {
					t.Fatal(block.Header.Height, in.PreviousOutPoint)
				}
			}
		}
	}
...
func TestExportData(t *testing.T) {
	...
	for i, block := range chain.Blocks() {
		...
		slot := uint64(block.Header.Timestamp.Unix()) / poc.PoCSlot
		quality, err := proof.GetVerifiedQuality(pocutil.PubKeyHash(pk), pocutil.Hash(block.Header.Challenge), slot, block.Header.Height)
		if err != nil {
			t.Fatal(err)
		}
		...
		poolArr[i-10] = PoolData{
			...
			// ChainCode:  chaincode,
			Proof: Proof{
				X:         hex.EncodeToString(block.Header.Proof.X),
				XPrime:    hex.EncodeToString(block.Header.Proof.XPrime),
				BitLength: block.Header.Proof.BitLength,
			},
			Target:      hex.EncodeToString(block.Header.Target.Bytes()),
			Quality:     hex.EncodeToString(quality.Bytes()),
			QualityData: proof.GetHashVal(uint64(block.Header.Timestamp.Unix())/poc.PoCSlot, block.Header.Height).String(),
			Challenge:   block.Header.Challenge.String(),
			Time:        uint64(block.Header.Timestamp.Unix()),
			Height:      block.Header.Height,
		}
	}
...
func TestMockDataForBlockchainTests(t *testing.T) {
	...
	case "6":
		testHelperExportblocks(t)
	default:
	}
}

// this is used for tests in package blockchain
func testHelperExportblocks(t *testing.T) {
	opt := &Option{
		Mode:                Auto,
		TotalHeight:         200,
		TxPerBlock:          6,
	...
	for _, block := range chain.Blocks() {
		buf, err := block.Bytes(wire.Packet)
		if err != nil {
			t.Fatal(err)
		}
...
func testExportForkBeforeStakingExpire(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			fmt.Println("total blocks: ", len(chain.allBlocks))
			for _, stat := range chain.stats {
				n := stat.stat[TxNormal]
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, block := range chain.allBlocks {
				...
				// 	fmt.Println(block.Header.Height, i, len(block.Transactions[i].TxOut))
				// }
			}
		})
	}
