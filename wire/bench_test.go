...
func BenchmarkEncodeBlock(b *testing.B) {
	...
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		_, err := block.Encode(bytes.NewBuffer([]byte("")), DB)
		if err != nil {
			b.Fatal(err)
		}
	}
...
func TestEncodeBlock(t *testing.T) {
	...
	start := time.Now()
	for i := 0; i < N; i++ {
		n, err := block.Encode(bytes.NewBuffer([]byte("")), DB)
		if err != nil {
			t.Fatal(i, err, n)
		}
	}
...
func BenchmarkDecodeBlock(b *testing.B) {
	...
	block := mockBlock(2000)
	var wBuf bytes.Buffer
	_, err := block.Encode(&wBuf, DB)
	if err != nil {
		b.Fatal(err)
	}
	buf := wBuf.Bytes()
	...
	var newblock = NewEmptyMsgBlock()
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		n, err := newblock.Decode(bytes.NewReader(buf), DB)
		if err != nil {
			b.Fatal(i, err, n)
		}
	}
