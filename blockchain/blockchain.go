...
func (chain *Blockchain) loadBlockNode(blockSha *wire.Hash) (*BlockNode, error) {
	blockHeader, err := chain.db.FetchBlockHeaderBySha(blockSha)
	if err != nil {
		return nil, err
	}

	node := NewBlockNode(blockHeader, blockSha, BFNone)
...
func (chain *Blockchain) getPrevNodeFromBlock(block *chainutil.Block) (*BlockNode, error) {
	// Genesis block.
...
func (chain *Blockchain) execProcessBlock(block *chainutil.Block, flags BehaviorFlags) (bool, error) {
...
func (chain *Blockchain) notifyBlockConnected(block *chainutil.Block) error {
	if block == nil {
...
func (chain *Blockchain) notifyTransactionReceived(tx *chainutil.Tx) error {
	if tx == nil {
		return errNilArgument
	}
	for listener := range chain.listeners {
