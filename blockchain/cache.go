...
func (cache *blockCache) addBlock(block *chainutil.Block) {
	cache.Lock()
	defer cache.Unlock()
...
func (cache *blockCache) getBlock(hash *wire.Hash) (*chainutil.Block, error) {
	cache.RLock()
	defer cache.RUnlock()
	...
	return chainutil.NewBlockFromBytes(bs, wire.Packet)
}
