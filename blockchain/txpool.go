...
type TxDesc struct {
	...
	totalInputValue  chainutil.Amount // total input value
}
...
type TxPool struct {
	sync.RWMutex
	pool          map[wire.Hash]*TxDesc // txSha --> TxDesc
...
func (tp *TxPool) IsTransactionInPool(txSha *wire.Hash) bool {
	...
	tp.RLock()
	defer tp.RUnlock()
	return tp.isTransactionInPool(txSha)
}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (tp *TxPool) removeTransactionFromAddrIndex(tx *chainutil.Tx) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (tp *TxPool) removeScriptFromAddrIndex(pkScript []byte, tx *chainutil.Tx) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (tp *TxPool) addTransaction(tx *chainutil.Tx, height uint64, startingPriority float64,
	totalInputValue, fee chainutil.Amount) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (tp *TxPool) addTransactionToAddrIndex(tx *chainutil.Tx) error {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (tp *TxPool) fetchReferencedOutputScripts(tx *chainutil.Tx) ([][]byte, error) {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (tp *TxPool) indexScriptAddressToTx(pkScript []byte, tx *chainutil.Tx) error {
	_, addresses, _, _, err := txscript.ExtractPkScriptAddrs(pkScript,
		&config.ChainParams)
	if err != nil {
		logging.CPrint(logging.ERROR, "Unable to extract encoded addresses from script for addrindex ",
...
func currentPriority(tx *chainutil.Tx, txStore TxStore, blockHeight uint64) (float64, chainutil.Amount, error) {
	inputAge, totalInputValue, err := calcInputValueAge(tx, txStore, blockHeight)
	if err != nil {
		return 0, chainutil.ZeroAmount(), err
	}
...
//
// This function MUST be called with the mempool lock held (for reads).
func (tp *TxPool) checkPoolDoubleSpend(tx *chainutil.Tx) error {
	for _, txIn := range tx.MsgTx().TxIn {
...
func (tp *TxPool) FilterTransactionsByAddress(addr chainutil.Address) ([]*chainutil.Tx, error) {
	...
	if txs, exists := tp.addrIndex[addr.EncodeAddress()]; exists {
		addressTxs := make([]*chainutil.Tx, 0, len(txs))
...
func (tp *TxPool) maybeAcceptTransaction(tx *chainutil.Tx, isNew, rateLimit bool) ([]*wire.Hash, error) {
	...
	requiredFee, err := CalcMinRequiredTxRelayFee(serializedSize, chainutil.MinRelayTxFee())
	if err != nil {
	...
	if txFee.Cmp(requiredFee) < 0 {
		...
		if rateLimit {
			...
			// minutes window - matches miner handling.
