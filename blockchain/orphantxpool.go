...
import (
	...
	"github.com/Sukhavati-Labs/go-miner/blockchain/orphanpool"
...
type orphanTx chainutil.Tx

func newOrphanTx(tx *chainutil.Tx) *orphanTx {
	return (*orphanTx)(tx)
}

func (tx *orphanTx) OrphanPoolID() string {
	return (*chainutil.Tx)(tx).Hash().String()
}

func (tx *orphanTx) Tx() *chainutil.Tx {
	return (*chainutil.Tx)(tx)
}

func (otp *OrphanTxPool) orphans() []*chainutil.Tx {
	entries := otp.pool.Items()
	orphans := make([]*chainutil.Tx, 0, len(entries))
...
func (otp *OrphanTxPool) addOrphan(tx *chainutil.Tx) {
...
func (otp *OrphanTxPool) maybeAddOrphan(tx *chainutil.Tx) error {
...
func (otp *OrphanTxPool) getOrphansByPrevious(hash *wire.Hash) []*chainutil.Tx {
	subs := otp.pool.ReadSubs(hash.String())
	orphans := make([]*chainutil.Tx, len(subs))
