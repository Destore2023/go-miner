...
func TestCalcMinRequiredTxRelayFee(t *testing.T) {
	tests := []struct {
		...
		{
			"max standard tx size with max Sukhavati relay fee",
			maxStandardTxSize,
			chainutil.MaxAmount().UintValue(),
			chainutil.MaxAmount().UintValue(),
		},
		{
	...
	for i, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			minFee, err := chainutil.NewAmountFromUint(test.relayFee)
			assert.Nil(t, err)
...
func TestCheckParsePkScript(t *testing.T) {
	var pubKeys [][]byte
	var witnessV0Addr chainutil.Address
	for i := 0; i < 4; i++ {
...
func TestCheckParsePkScript3(t *testing.T) {
	...
	pocpkh := chainutil.Hash160(pk.PubKey().SerializeCompressed())
	fmt.Println(pocpkh)
		...
		},
		{
			...
			TxStore{
				wire.Hash{220, 135, 137, 12, 160, 115, 62, 251, 6, 162, 111, 181, 149, 164, 14, 184, 28, 95, 47, 39, 186, 242, 48, 135, 50, 163, 6, 205, 179, 57, 115, 109}: &TxData{
					Tx: chainutil.NewTx(txCoinbaseBinding),
				},
			},
			false,
...
func TestDust(t *testing.T) {
	...
	amt1, err := chainutil.NewAmountFromUint(1)
	assert.Nil(t, err)
	amtMinRelay, err := chainutil.NewAmountFromUint(consensus.MinRelayTxFee)
	assert.Nil(t, err)
	...
	tests := []struct {
		...
		},
		{
			...
			"max Sukhavati amount is never dust",
			wire.TxOut{Value: chainutil.MaxAmount().IntValue(), PkScript: pkScript},
			chainutil.MaxAmount(),
			false,
		},
		{
			...
			chainutil.ZeroAmount(), // no relay fee
			true,
		},
	}
	for i, test := range tests {
...
func TestCheckInputsStandard(t *testing.T) {
	...
	block, err := loadNthBlock(23)
	assert.Equal(t, uint64(22), block.Height())
	assert.Nil(t, err)
...
func TestCheckTransactionStandard(t *testing.T) {
	...
	addr, err := chainutil.NewAddressWitnessScriptHash(witnessV0Addr.ScriptAddress(),
		&config.ChainParams)
	if err != nil {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			err := checkTransactionStandard(chainutil.NewTx(&test.tx),
				test.height, chainutil.MinRelayTxFee(), txStore)
