...
func CalcMinRequiredTxRelayFee(serializedSize int64, minTxRelayFee chainutil.Amount) (chainutil.Amount, error) {
	...
	// free transaction relay fee). minTxRelayFee is in Sukhavati/kB so
	...
	amt, err := chainutil.NewAmount(required)
	if err == chainutil.ErrMaxAmount {
		return chainutil.MaxAmount(), nil
	}
	return amt, err
}
...
func calcPriority(tx *chainutil.Tx, inputValueAge *safetype.Uint128) float64 {
...
func calcInputValueAge(tx *chainutil.Tx, txStore TxStore, nextBlockHeight uint64) (*safetype.Uint128, chainutil.Amount, error) {

	totalInputAge := safetype.NewUint128()
	totalInputValue := chainutil.ZeroAmount()
	var err error
	for _, txIn := range tx.MsgTx().TxIn {
...
func checkInputsStandard(tx *chainutil.Tx, txStore TxStore) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee chainutil.Amount) (bool, error) {
	...
	// minFreeTxRelayFee is in Sukhavati/KB, so multiply by 1000 to
	// convert to bytes.
	//
	// Using the typical values for a pay-to-pubkey-hash transaction from
	// the breakdown above and the default minimum free transaction relay
	// fee of 1000, this equates to values less than 546 Sukhavati being
	// considered dust.
	//
