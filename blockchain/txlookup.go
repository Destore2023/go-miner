...
func connectTransactions(txStore TxStore, block *chainutil.Block) error {
...
// transactions in the passed map are updated.
func disconnectTransactions(db database.DB, txStore TxStore, block *chainutil.Block) error {
	// Loop through all of the transactions in the block to see if any of
	...
	for i := len(block.Transactions()) - 1; i >= 0; i-- {
		...
		// Unspent the origin transaction output.
...
func fetchTxStoreMain(db database.DB, txSet map[wire.Hash]struct{}, includeSpent bool) TxStore {
	...
	for _, txReply := range txReplyList {
		...
		txD.Err = txReply.Err
		if txReply.Err == nil {
			txD.Tx = chainutil.NewTx(txReply.Tx)
...
func (chain *Blockchain) fetchInputTransactions(node *BlockNode, block *chainutil.Block) (TxStore, error) {
	...
	for i, tx := range transactions {
		...
		for _, txIn := range txIns {
			...
			// store accordingly when this is the case.  Otherwise,
...
func connectStakingTransactions(txStore database.StakingNodes, block *chainutil.Block) error {
	// Loop through all of the transactions in the block to see if any of
	// them are ones we need to update and spend based on the results map.
	for _, tx := range block.Transactions() {
		// Update the transaction store with the transaction information
...
func disconnectStakingTransactions(db database.DB, txStore database.StakingNodes, block *chainutil.Block) error {
	// Loop through all of the transactions in the block to see if any of
	// them are ones that need to be undone based on the transaction store.
...
func (chain *Blockchain) fetchStakingTxStore(node *BlockNode) ([]database.Rank, error) {
	...
	// Fetch the requested set from the point of view of the end of the
	// main (best) chain including fully spent transactions.  The fully
	// spent transactions are needed because the following code unspent
	// them to get the correct point of view.
...
func (chain *Blockchain) fetchStakingPoolTxStore(node *BlockNode) {
