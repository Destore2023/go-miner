...
const (
	txIndexKeyLen       = sha256.Size + 4 + 4                    // 40
	btxIndexKeyLen      = ripemd160.Size + 4 + 4 + 4             // 32
	btxSpentIndexKeyLen = ripemd160.Size + 4 + 4 + 8 + 4 + 4 + 4 // 48
)
...
func (a *AddrIndexer) insertTxAddressIndex(block *chainutil.Block, txStore TxStore) error {
...
func (a *AddrIndexer) deleteTxAddressIndex(blockSha *wire.Hash, blockHeight uint64) error {
	currentIndexSha, currentIndexTip, err := a.db.FetchAddrIndexTip()
	if err != nil {
		return err
	}
	if blockHeight == currentIndexTip && blockSha.IsEqual(currentIndexSha) {
		err := a.db.DeleteAddrIndex(blockSha, blockHeight)
		if err != nil {
			logging.CPrint(logging.ERROR, "Unable to write index for block",
				logging.LogFormat{
					"block hash": blockSha.String(),
					"height":     blockHeight,
					"error":      err,
				})
			a.stopServer()
...
func (a *AddrIndexer) indexBlockAddrs(block *chainutil.Block, txStore TxStore) (database.TxAddrIndex, database.BindingTxAddrIndex, database.BindingTxSpentAddrIndex, error) {
	...
	for txIdx, tx := range block.Transactions() {
		...
		if !IsCoinBase(tx) {
			...
			for _, txIn := range tx.MsgTx().TxIn {
				...
				blockHeightBefore, txOffsetBefore, txLenBefore, err := a.db.GetUnspentTxData(&prevOut.Hash)
				if err != nil {
					...
					blockHeightBefore = block.Height()
				...
				err = indexScriptPubKeyForTxIn(txAddrIndex, btxSpentIndex, txD.Tx.MsgTx().TxOut[prevOut.Index].PkScript, locInBlock, blockHeightBefore, txBeforeLoc, prevOut.Index)
				if err != nil {
...
func (a *AddrIndexer) logBlockHeight(block *chainutil.Block) {
	a.blockLogger.LogBlockHeight(block)
}
