...
var testTxs map[string]*block50Tx

type block50Tx struct {
...
func init() {
	...
	testTxs = make(map[string]*block50Tx)
	for _, tx := range slc {
		testTxs[tx.Name] = tx
...
func newTxPool(loadBlocksNum int) (*TxPool, func(), error) {
	...
	err = loadTestBlocksIntoTestChainDb(db, loadBlocksNum)
	if err != nil {
		db.Close()
		return nil, nil, err
	}
	...
	bc.GetTxPool().SetNewTxCh(make(chan *chainutil.Tx, 50))
...
func TestTxPool_checkPoolDoubleSpend(t *testing.T) {
	...
	assert.Nil(t, err)
	txP.addTransaction(chainutil.NewTx(childTx), 26, 100.0, chainutil.ZeroAmount(), chainutil.ZeroAmount())

	grandTx, err := getTx("grandChild")
	assert.Nil(t, err)

	tx := chainutil.NewTx(grandTx)
	txP.addTransaction(chainutil.NewTx(grandTx), 27, 100.0, chainutil.ZeroAmount(), chainutil.ZeroAmount())
...
func TestTxPool_MaybeAcceptTransaction_totalSukhavati(t *testing.T) {
	txP, close, err := newTxPool(25)
	if err != nil {
		t.Error(err)
	}
	...
	tx := chainutil.NewTx(msgtx)
	tx.MsgTx().TxOut[len(tx.MsgTx().TxOut)-1].Value = 1000000000000 // 10000 skt
...
func TestTxPool_MaybeAcceptTransaction_CoinbaseTx(t *testing.T) {
	...
	block, err := loadNthBlock(25)
	assert.Nil(t, err)

	coinbaseMsgTx := block.MsgBlock().Transactions[0]
	tx := chainutil.NewTx(coinbaseMsgTx)
...
func Test_calcMinRequiredTxRelayFee(t *testing.T) {
	...
	minFee, err := CalcMinRequiredTxRelayFee(serializedSize, chainutil.MinRelayTxFee())
	assert.Nil(t, err)
	assert.Equal(t, minFee.IntValue(), serializedSize*int64(consensus.MinRelayTxFee/1000))

	// settings for test
	oldLimit := config.FreeTxRelayLimit
	oldFee := chainutil.MinRelayTxFee()
	config.FreeTxRelayLimit = 0.0
	chainutil.TstSetMinRelayTxFee(10000000000)
	defer func() {
		config.FreeTxRelayLimit = oldLimit
		chainutil.TstSetMinRelayTxFee(oldFee.UintValue())
	}()

	_, err = txP.maybeAcceptTransaction(chainutil.NewTx(tx), true, true)
...
func TestIsFinalizedTransaction(t *testing.T) {
	...
	_, err = txP.maybeAcceptTransaction(tx, true, true)
	assert.Equal(t, ErrUnfinishedTx, err)
}
...
func TestTxPool_addTransaction(t *testing.T) {
	...
	txP.addTransaction(tx, 22, 12, chainutil.ZeroAmount(), chainutil.ZeroAmount())
...
func TestTxPool_RemoveTransaction(t *testing.T) {
	...
	txP.addTransaction(tx, 22, 22, chainutil.ZeroAmount(), chainutil.ZeroAmount())
	assert.True(t, txP.haveTransaction(tx.Hash()))
...
func TestTxPool_RemoveDoubleSpends(t *testing.T) {
	...
	txP.addTransaction(tx, 12, 12, chainutil.ZeroAmount(), chainutil.ZeroAmount())
...
func TestTxPool_FilterTransactionsByAddress(t *testing.T) {
	...
	err = txP.addTransaction(tx, 25, 25, chainutil.ZeroAmount(), chainutil.ZeroAmount())
	assert.Nil(t, err)
...
func TestTxPool_ProcessTransaction(t *testing.T) {
	...
	assert.Nil(t, err)
	tx3 := chainutil.NewTx(msgtx3)

	msgtx2, err := getTx("child1")
	assert.Nil(t, err)
	tx2 := chainutil.NewTx(msgtx2)
