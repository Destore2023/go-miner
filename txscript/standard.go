...
// address.
func PayToStakingAddrScript(addr chainutil.Address, frozenPeriod uint64) ([]byte, error) {
...
func ExtractPkScriptAddrs(pkScript []byte, chainParams *config.Params) (ScriptClass, []chainutil.Address, []*btcec.PublicKey, int, error) {
	...
	case StakingScriptHashTy:
		requiredSigs = 1
		addr, err := chainutil.NewAddressStakingScriptHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	case BindingScriptHashTy:
		...
		pkAddr, err := chainutil.NewAddressPubKeyHash(pops[2].data,
			chainParams)
		if err == nil {
